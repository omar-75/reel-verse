...
function displayMovies() {
    ...
    // Single Element find for lopp
    for (let i = 0; i < moviesArr.length; i++) {
        // console.log(moviesArr[i]);
        const moviesTitle = moviesArr[i].title;
        const price = moviesArr[i].ticketPrice;
        const posterURL = moviesArr[i].posterURL;
        ...
        <img src="${posterURL}" class="card-img-top" alt="movie">
      </div>
        <div class="card-body" >
          <h5 class="card-title h5 fw-bold mb-2">${moviesTitle}</h5>
          <h5 class="card-text text-danger fw-medium"> ${price} ৳</h5>
...
function displayModel() {
    ...
    const bookTricket = document.querySelectorAll('.book-tricket ');

    for (let i = 0; i < bookTricket.length; i++) {
        bookTricket[i].addEventListener('click', function () {
